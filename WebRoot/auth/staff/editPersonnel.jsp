...
<%
	...
	if (null == pidString || 1 > pidString.length()) {
		if (null == (pidString = (String) session
				.getAttribute("editmid"))) {
	...
	EditPersonnelAction personnelEditor = new EditPersonnelAction(
			prodDAO, loggedInMID.longValue(), pidString);
	long pid = personnelEditor.getPid();

	/* Now take care of updating information */
	boolean formIsFilled = (null != request
			.getParameter("formIsFilled"))
			&& request.getParameter("formIsFilled").equals("true");
	PersonnelBean personnelForm;
	if (formIsFilled) {
		personnelForm = new BeanBuilder<PersonnelBean>().build(
				request.getParameterMap(), new PersonnelBean());
		try {
			/* if (personnelForm.getStreetAddress1().length() > 20
					|| personnelForm.getStreetAddress2().length() > 20) {
		  */
				personnelEditor.updateInformation(personnelForm);
				if (personnelForm.getRole() == Role.HCP) // If pid belongs to an HCP
					loggingAction.logEvent(TransactionType.LHCP_EDIT,
							loggedInMID.longValue(),
							personnelForm.getMID(), "");
				else if (personnelForm.getRole() == Role.UAP) // If pid belongs to a UAP
					loggingAction.logEvent(TransactionType.UAP_EDIT,
							loggedInMID.longValue(),
							personnelForm.getMID(), "");
				else if (personnelForm.getRole() == Role.ER) // If pid belongs to a ER
					loggingAction.logEvent(TransactionType.ER_EDIT,
							loggedInMID.longValue(),
							personnelForm.getMID(), "");
				else if (personnelForm.getRole() == Role.PHA) // If pid belongs to a PHA
					loggingAction.logEvent(TransactionType.PHA_EDIT,
							loggedInMID.longValue(),
							personnelForm.getMID(), "");
				else if (personnelForm.getRole() == Role.LT) // If pid belongs to a LT
					loggingAction.logEvent(TransactionType.LT_EDIT,
							loggedInMID.longValue(),
							personnelForm.getMID(), "");
...
<%
	...
	loggingAction.logEvent(TransactionType.PERSONNEL_VIEW,
			loggedInMID.longValue(), personnelForm.getMID(), "");
%>
<br>

<form action="editPersonnel.jsp" method="post">
	<input type="hidden" name="formIsFilled" value="true"> <input
		type="hidden" name="roleString" value="<%=personnelForm.getRole()%>" />
	<div align=center>
		<table class="fTable" align=center>
			...
			<tr>
				...
				<td><input name="streetAddress1"
					value="<%=StringEscapeUtils.escapeHtml(""
					+ (personnelForm.getStreetAddress1()))%>"
					type="text"><br /> <input name="streetAddress2"
					value="<%=StringEscapeUtils.escapeHtml(""
					+ (personnelForm.getStreetAddress2()))%>"
					type="text"></td>
			</tr>
			<tr>
				...
				<td><input name="city"
					value="<%=StringEscapeUtils.escapeHtml(""
					+ (personnelForm.getCity()))%>"
					type="text"></td>
			</tr>
			<tr>
				<td class="subHeaderVertical">State:</td>
				<td><itrust:state name="state"
						value="<%=StringEscapeUtils.escapeHtml(personnelForm.getState())%>" />
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input name="email"
					value="<%=StringEscapeUtils.escapeHtml(""
					+ (personnelForm.getEmail()))%>"
					type="text"></td>
			</tr>
			<input name="specialty"  value="<%= StringEscapeUtils.escapeHtml("" + (personnelForm.getSpecialty())) %>"  type="hidden" >

		</table>
		</td>
		</tr>
		<tr>
			<td colspan=2><br /> <input type="submit" name="action"
				style="font-size: 16pt; font-weight: bold;"
				value="Edit Personnel Record"></td>
		</tr>
		</table>
	</div>
</form>
<div align=center>
	<br /> <span style="font-size: 14px;"> Note: in order to set
		the password for this user, use the "Reset Password" link at the login
		page. </span> <br />
</div>
